...
M.Coverage = function(fn, html)
	...
	local relativeFile = string.gsub(vim.fn.fnamemodify(vim.fn.expand("%"), ":."), "_test", "")
	...
	h = vim.loop.spawn(
		...
		vim.schedule_wrap(function(code, signal)
			...
			if code == 0 then
				local percent =
					string.gmatch(table.concat(vim.api.nvim_buf_get_lines(M.errBuf, 0, -1, true)), "coverage: (%d+)")()
				if percent ~= nil then
				...
				for i = 2, #lines do
					local lineData = parse_line(lines[i])
					...
					if string.find(path, relativeFile) then
						...
						local startcol = math.max(tonumber(marks()), 0)
